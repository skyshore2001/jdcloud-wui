...
{$.parser.parse(jpageNew);enhanceGrid(jpageNew);self.enhanceWithin(jpageNew);callInitfn(jpageNew,paramArr);jpageNew.data("showPageArgs_",showPageArgs_);jpageNew.trigger('pagecreate');jpageNew.trigger('pageshow');}}
...
jdlg=$(jdlg);if(opt&&opt.reload){opt=$.extend({},opt);delete opt.reload;if(jdlg.size()>0)
unloadDialog(jdlg);}
...
var jtbl=jdlg.jdata().jtbl;var obj=opt.url.action.replace(".set","");var rv=batchOp(obj,obj+".setIf",jtbl,{acName:"更新",data:data,onBatchDone:function(){closeDlg(jdlg);}});if(rv!==false)
return;}
...
return false;opt=$.extend({batchOpMode:0,acName:"操作"},opt);var selArr=jtbl.datagrid("getChecked");var batchOpMode=opt.batchOpMode;if(!batchOpMode&&!(m_batchMode||selArr.length>1)){return false;}
if(batchOpMode===2&&!m_batchMode&&selArr.length==0){self.app_alert("请先选择一行。","w");return false;}
var queryParams;var doBatchOnSel=selArr.length>1&&selArr[0].id!=null;var acName=opt.acName;if(!doBatchOnSel&&batchOpMode===2&&!m_batchMode&&selArr.length==1&&selArr[0].id!=null)
doBatchOnSel=true;if(doBatchOnSel){if(selArr.length==1){queryParams={cond:"id="+selArr[0].id};}
...
return;function confirmBatch(batchCnt){console.log(ac+": "+JSON.stringify(queryParams));if(batchCnt==0){app_alert("没有记录需要操作。");return;}
...
function doBatch(data){self.callSvr(ac,queryParams,function(cnt){opt.onBatchDone&&opt.onBatchDone();if(doBatchOnSel&&selArr.length==1){reloadRow(jtbl,selArr[0]);}
else{reload(jtbl);}
...
loadDialogTpl1();function loadDialogTpl1()
{$.parser.parse(jdlg);jdlg.find(">table:first, form>table:first").has(":input").addClass("wui-form-table");self.enhanceWithin(jdlg);var val=jdlg.attr("wui-script");if(val!=null){var path=getModulePath(val);var dfd=mCommon.loadScript(path,onLoad);dfd.fail(function(){self.app_alert("加载失败: "+val);});}
...
{mCommon.assert(jd.jtbl);if(mode==FormMode.forDel){var rv=batchOp(obj,obj+".delIf",jd.jtbl,{acName:"删除",onBatchDone:onCrud});if(rv!==false)
return;}
...
self.reloadRow(opt.jtbl,rowData);};var showDlgOpt=$.extend({},opt,{url:url,okLabel:BTN_TEXT[mode],validate:mode!=FormMode.forFind,modal:false,reset:doReset,data:load_data,onSubmit:onSubmit,onOk:onOk,objParam:opt});showDlg(jdlg,showDlgOpt);if(mode==FormMode.forSet)
...
var dfd=self.callSvr(opts.url,param1,success);dfd.fail(function(){jo.datagrid("loaded");});}
