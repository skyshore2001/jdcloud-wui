...
opt=$.extend({okLabel:"确定",cancelLabel:"取消",noCancel:false,modal:true,reset:true,validate:true},opt);jdlg.addClass('wui-dialog');callInitfn(jdlg);var jfrm=jdlg.find("form:first");var formMode=jdlg.jdata().mode;jfrm.trigger("beforeshow",[formMode,opt]);var btns=[{text:opt.okLabel,iconCls:'icon-ok',handler:fnOk}];if(!opt.noCancel)
btns.push({text:opt.cancelLabel,iconCls:'icon-cancel',handler:fnCancel})
if($.isArray(opt.buttons))
btns.push.apply(btns,opt.buttons);var dlgOpt={maximizable:true,collapsible:true,resizable:true,left:null,top:null,closable:!opt.noCancel,modal:opt.modal,buttons:btns};if(jdlg.is(":visible")){dlgOpt0=jdlg.dialog("options");$.extend(dlgOpt,{left:dlgOpt0.left,top:dlgOpt0.top});}
...
return false;var ev=$.Event("validate");jfrm.trigger(ev,[formMode,opt.data]);if(ev.isDefaultPrevented())
return false;var ev=$.Event("savedata");jfrm.trigger(ev,[formMode,opt.data]);if(ev.isDefaultPrevented())
return false;if(opt.url){var data=mCommon.getFormData(jdlg);if(opt.onSubmit&&opt.onSubmit(data)===false)
return false;self.callSvr(opt.url,success,data);}
else{opt.onOk&&opt.onOk.call(jdlg);}
...
var id=opt.id;var obj=jdlg.attr("my-obj");mCommon.assert(obj);var jd=jdlg.jdata();var jd2=jd.jtbl&&jd.jtbl.jdata();var rowData;if(id==null){if(mode==FormMode.forSet||mode==FormMode.forDel)
...
else if(mode==FormMode.forSet){url=self.makeUrl([obj,"set"],{id:id});}
...
else{je.prop("disabled",false);je.addClass("mui-find-field");je.prop("title",self.queryHint);var type=je.attr("type");if(type&&["number","date","time","datetime"].indexOf(type)>=0){bak.type=type;je.attr("type","text");}}});jfrm.find(".easyui-validatebox").validatebox("disableValidation");}
else if(jd.mode==FormMode.forFind&&mode!=FormMode.forFind){jfrm.find(":input[name]").each(function(i,e){var je=$(e);var bak=je.jdata().bak;je.prop("disabled",bak.disabled);je.removeClass("mui-find-field");je.prop("title",bak.title);if(bak.type){je.attr("type",bak.type);}})
...
load_data={};}
else if(mode==FormMode.forSet){if(rowData){load_data=add_(rowData);}
else{var load_url=self.makeUrl([obj,'get'],{id:id});var data=self.callSvrSync(load_url);if(data==null)
return;load_data=add_(data);}}
showDlg(jdlg,{url:url,okLabel:BTN_TEXT[mode],validate:mode!=FormMode.forFind,modal:false,reset:doReset,data:load_data,onSubmit:onSubmit,onOk:onOk});if(mode==FormMode.forSet)
jfrm.form("validate");function onSubmit(data){if(mode==FormMode.forSet){if($.isEmptyObject(data)){closeDlg(jdlg);return false;}}}
function onOk(retData){if(mode==FormMode.forFind){var param=getFindData(jfrm);mCommon.assert(jd.jtbl);var dgOpt=jd.jtbl.datagrid("options");if(param.cond&&dgOpt&&dgOpt.url&&dgOpt.url.params&&dgOpt.url.params.cond){param.cond=dgOpt.url.params.cond+" AND ("+param.cond+")";}
reload(jd.jtbl,undefined,param);return;}
if(jd.jtbl){if(mode==FormMode.forSet&&rowData)
...
return function(){var url=WUI.makeUrl(ac,getQueryParamFromTable(jtbl,param));window.open(url);}}
...
if(param.fname===undefined){if(jtbl.attr("title")){param.fname=jtbl.attr("title");}}
return param;}
...
return"(无图)";return value.replace(/(\d+),?/g,function(ms,picId){var url=WUI.makeUrl("att",{thumbId:picId});return"<a target='_black' href='"+url+"'>"+picId+"</a>&nbsp;";});},flag:function(yes,no){if(yes==null)
yes="是";if(no==null)
no="否";return function(value,row){if(value==null)
return;return value?yes:no;}},enum:function(enumMap){return function(value,row){if(value==null)
return;return enumMap[value]||value;}},linkTo:function(field,dlgRef){return function(value,row){if(value==null)
...
{var type="i";var fn=undefined;var alertOpt={};for(var i=1;i<arguments.length;++i){var arg=arguments[i];if($.isFunction(arg)){fn=arg;}
...
else if(type=="p"){$.messager.prompt(self.options.title,msg,function(text){if(text&&fn){fn(text);}});setTimeout(function(){var ji=$(".messager-window .messager-input");ji.focus();if(alertOpt.defValue){ji.val(alertOpt.defValue);}});return;}
var icon={i:"info",w:"warning",e:"error"}[type];var s={i:"提示",w:"警告",e:"出错"}[type]||"";var s1="<b>["+s+"]</b>";$.messager.alert(self.options.title+" - "+s,s1+" "+msg,icon,fn);setTimeout(function(){var jbtn=$(".messager-window .l-btn");jbtn.focus();if(alertOpt.timeoutInterval){setTimeout(function(){jbtn.click();},alertOpt.timeoutInterval);}});}
...
{if(text==null)
text=id;return"<a href=\""+dlg+"\" onclick='WUI.showObjDlg(\""+dlg+"\",FormMode.forSet,{id:"+id+"});return false'>"+text+"</a>";}
function tokenName()
...
return;jpage.trigger('pagedestroy');}});var tmr;$(window).on("resize",function(){if(tmr)
clearTimeout(tmr);tmr=setTimeout(function(){tmr=null;console.log("panel resize");var jpage=getActivePage();jpage.closest(".panel-body").panel("doLayout",true);},200);});}
$(mainInit);}
...
jo.on("refresh",refresh);jo.on("markRefresh",markRefresh);}
...
if(opts.url==null)
return;if(opts.dirty||m_dataCache[opts.url]===undefined){self.callSvr(opts.url,onLoadOptions);}
else{onLoadOptions(m_dataCache[opts.url]);}
function onLoadOptions(data){applyData(data);jo.val(jo.prop("value_"));}}
function applyData(data)
{m_dataCache[opts.url]=data;opts.dirty=false;function getText(row)
...
{var hook=$.valHooks["select"];$.valHooks["select"]={set:function(elem,value){elem.value_=value;return hook.set.apply(this,arguments);},get:hook.get}}
