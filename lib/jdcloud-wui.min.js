...
setFixedFields(jdlg,opt);focusDlg(jdlg);jfrm.trigger("show",[formMode,opt.data]);opt.onShow&&opt.onShow(formMode,opt.data);function fnCancel(){closeDlg(jdlg)}
function fnOk()
...
{if(opt.data)
$.extend(opt.data,reqData);if(opt.onOk){jfrm.trigger('retdata',[retData,formMode]);if(opt.onOk==='close'){app_show("操作成功!");WUI.closeDlg(jdlg);}
...
var cleanFn=[];var forFind=beforeShowOpt.mode==FormMode.forFind;self.formItems(jfrm,function(ji,name,it){var fixedVal=(ji.hasClass("wui-fixedField")&&objParam&&objParam[name]!=null)?objParam[name]:null;var isOld=false;if(fixedVal!=null){isOld=true;}
else{fixedVal=(fixedFields&&fixedFields[name]!=null)?fixedFields[name]:null;}
if(fixedVal!=null){var oldVal=it.getReadonly();it.setReadonly(true);if(forFind){var oldVal2=it.getDisabled();it.setDisabled(true);}
if(isOld){it.setValue(fixedVal);}
cleanFn.push(function(){it.setReadonly(oldVal);if(forFind){it.setDisabled(oldVal2);}});}});if(cleanFn.length>0)
...
var BTN_TEXT=["添加","保存","保存","查找","删除"];function getFindData(jfrm,doGetAll)
...
return;opt=opt||{};if(!jdlg.selector){jdlg.addClass('wui-dialog');var jcontainer=$("#my-pages");jdlg.appendTo(jcontainer);loadDialogTpl1();return true;}
...
{var obj=opt.obj||jdlg.attr("my-obj");var meta=opt.meta||(obj&&self.UDF.onGetMeta(obj));if(meta){var jp=jdlg.find(opt.metaParent||"table:first");if(jp.size()>0&&!jp.hasClass("wui-meta-parent")){self.UDF.addFieldByMeta(jdlg,jp,meta);jp.addClass("wui-meta-parent");}}
...
return;it.setDisabled(bak.disabled);it.setReadonly(bak.readonly);var jshow=it.getShowbox();jshow.removeClass("wui-find-field")
jshow.prop("title",bak.title);if(bak.type){jshow.attr("type",bak.type);}});jfrm.form("enableValidation");}
...
var jchkClose=null;var showDlgOpt=$.extend({},opt,{url:url,okLabel:BTN_TEXT[mode],validate:mode!=FormMode.forFind,modal:false,reset:doReset,data:load_data,onOk:onOk,objParam:opt,onShow:onShow});showDlg(jdlg,showDlgOpt);if(mode==FormMode.forSet)
jfrm.form("validate");function onShow(formMode,data){var jbtns=jdlg.next(".dialog-button");jchkClose=jbtns.find(".chkClose");if(jchkClose.size()==0){var jo=$("<label style='float:left'><input type='checkbox' class='chkClose'> 确定后关闭</label>").prependTo(jbtns);jchkClose=jo.find(".chkClose");if(formMode==FormMode.forAdd){var val=jdlg.data("closeAfterAdd");if(val||self.options.closeAfterAdd)
...
function onOk(retData){opt.onOk&&opt.onOk(retData);var jtbl=jd.jtbl;mCommon.assert(jchkClose.size()>0);var doClose=jchkClose.prop("checked");if(mode==FormMode.forFind){mCommon.assert(jtbl);reload(jtbl,undefined,retData);if(doClose)
...
return;var one=e.field+" \""+e.title+"\"";if(e.jdEnumMap){one+='='+mCommon.kvList2Str(e.jdEnumMap,';',':');}
...
return res;}
window.YesNoMap={0:"否",1:"是"};window.YesNo2Map={0:"否",1:"是",2:"处理中"};var Formatter={dt:function(value,row){var dt=WUI.parseDate(value);if(dt==null)
...
+'</div>';return htmlstr;}};self.formatter=Formatter;$.extend($.fn.combobox.defaults,{valueField:'val',textField:'text'});function dgLoader(param,success,error)
...
{var ret=jdListToDgList(data);var isOnePage=(ret.total==ret.rows.length);$(this).datagrid("getPager").toggle(!(isOnePage&&ret.total<=5));var dgOpt=$(this).datagrid("options");if(dgOpt.showFooter&&dgOpt.sumFields){var stat=data.stat||{};dgOpt.sumFields.forEach(function(field){if(stat[field]!==undefined)
...
clearTimeout(this_.infoTmr);this_.infoTmr=setTimeout(function(){GridHeaderMenu.statSelection(jtbl,row,field);delete this_.infoTmr;},1000);},onInitOptions:function(opt){var ac=opt.url&&opt.url.action;if(ac){var m=ac.match(/\w+(?=\.query\b)/);var obj=m&&m[0];if(obj){opt.obj=obj;var meta=self.UDF.onGetMeta(obj);if(meta&&meta.defaultFlag){self.UDF.addColByMeta(opt.columns[0],meta);}}}},});var GridHeaderMenu={showMenu:function(pos,jtbl,field){jmenu=$('<div class="mnuGridHeader"></div>');var items=field?GridHeaderMenu.itemsForField:GridHeaderMenu.items;$.each(items,function(i,e){var je=$(e);var perm=je.attr("wui-perm")||je.text();if(canDo("通用",perm))
je.appendTo(jmenu);});jmenu.menu({onClick:function(mnuItem){GridHeaderMenu[mnuItem.id].call(mnuItem,jtbl,field);}});jmenu.menu('show',pos);},items:['<div id="showDlgFieldInfo">字段信息</div>','<div id="showDlgDataReport" data-options="iconCls:\'icon-sum\'">自定义报表</div>','<div id="showDlgQuery" data-options="iconCls:\'icon-search\'">自定义查询</div>','<div id="import" wui-perm="新增" data-options="iconCls:\'icon-add\'">导入</div>','<div id="export" data-options="iconCls:\'icon-save\'">导出</div>'],itemsForField:['<div id="copyCol">复制本列</div>','<div id="statCol" data-options="iconCls:\'icon-sum\'">统计本列</div>','<div id="doFindCell" data-options="iconCls:\'icon-search\'">查询本列</div>',],showDlgFieldInfo:function(jtbl){var param=WUI.getQueryParamFromTable(jtbl);console.log(param);var title="字段信息";var title1=jtbl.prop("title")||jtbl.closest(".wui-page").prop("title");if(title1)
...
strArr.push("<b>[接口]</b>\n"+url.action);if(param.cond)
strArr.push("<b>[查询条件]</b>\n"+param.cond);if(param.orderby)
strArr.push("<b>[排序]</b>\n"+param.orderby);strArr.push("<b>[字段列表]</b>\n"+param.res.replace(/,/g,"\n"));var jdlg=$("<div title='"+title+"'><pre>"+strArr.join("\n\n")+"</pre></div>");WUI.showDlg(jdlg,{modal:false,onOk:function(){WUI.closeDlg(this);},noCancel:true});},showDlgDataReport:function(jtbl){self.showDlg("#dlgDataReport");},showDlgQuery:function(jtbl){var data=null;var datagrid=WUI.isTreegrid(jtbl)?"treegrid":"datagrid";var url=jtbl[datagrid]("options").url;if(url&&url.action)
data={ac:url.action};var param=WUI.getQueryParamFromTable(jtbl);self.showDlgQuery(data,param);},'import':function(jtbl){var param=self.getDgInfo(jtbl);if(!param.obj){app_alert("该数据表不支持导入","w");return;}
...
var stat=GridHeaderMenu.dgStatCol(data.rows,field);stat.info.unshift("<b>["+colTitle+"]</b>列:");var jdlg=$("<div title='列统计'><pre>"+stat.info.join("\n")+"</pre></div>");WUI.showDlg(jdlg,{modal:false,onOk:function(){WUI.closeDlg(this);},noCancel:true});},doFindCell:function(jtbl,field){var row=WUI.getRow(jtbl);if(row==null)
return;var param={cond:{}}
param.cond[field]=row[field];var doAppendFilter=WUI.isBatchMode();WUI.reload(jtbl,undefined,param,doAppendFilter);console.log("查询(按住Ctrl点击可追加查询条件): ",param.cond);}}
self.GridHeaderMenu=GridHeaderMenu;self.showDlgQuery=showDlgQuery;function showDlgQuery(data1,param)
{var itemArr=[{title:"接口名",dom:"<input name='ac' required>",hint:"示例: Ordr.query"},{title:"参数",dom:'<textarea name="param" rows=8></textarea>',hint:"cond:查询条件, res:返回字段, gres:分组字段, pivot:转置字段, fmt:输出格式(html,excel,txt,list,array,csv等)"}];var data=$.extend({ac:'Ordr.query',param:param?JSON.stringify(param,null,2):'{\n cond: {createTm: ">2020-1-1"},\n res: "count(*) 数量",\n gres: "status 状态=CR:新创建;PA:待处理;RE:已完成;CA:已取消",\n// pivot: "状态"\n}'},data1);self.showDlgByMeta(itemArr,{title:"高级查询",modal:false,data:data,onOk:function(data){var param={page:1};if(data.param){try{param=$.extend(param,eval("("+data.param+")"));}
...
var opt=$(this).tree("options");var ret=WUI.makeTree(arr,opt.idField,opt.fatherField);$.each(arr,function(i,e){e.text=e[opt.textField];});return ret;}});$.extend($.fn.combogrid.defaults,{loadFilter:$.fn.datagrid.defaults.loadFilter,loader:$.fn.datagrid.defaults.loader,});$.extend($.fn.combotreegrid.defaults,{idField:"id",treeField:"name",textField:"name",fatherField:"fatherId",loadFilter:$.fn.treegrid.defaults.loadFilter});var DefaultValidateRules={number:{validator:function(v){return v.length==0||/^[0-9.-]+$/.test(v);},message:'必须为数字!'},uname:{validator:function(v){return v.length==0||(v.length>=4&&v.length<=16&&/^[a-z]\w+$/i.test(v));},message:"4-16位英文字母或数字，以字母开头，不能出现符号."},pwd:{validator:function(v){return v.length==0||(v.length>=4&&v.length<=16)||v.length==32;},message:"4-16位字母、数字或符号."},equalTo:{validator:function(v,param){return v.length==0||v==$(param[0]).val();},message:"两次输入不一致."},cellphone:{validator:function(v){return v.length==0||(v.length==11&&!/\D/.test(v));},message:"手机号为11位数字"},datetime:{validator:function(v){return v.length==0||/\d{4}-\d{1,2}-\d{1,2}( \d{1,2}:\d{1,2}(:\d{1,2})?)?/.test(v);},message:"格式为\"年-月-日 时:分:秒\"，时间部分可忽略"},usercode:{validator:function(v){return v.length==0||/^[a-zA-Z]/.test(v)||(v.length==11&&!/\D/.test(v));},message:"11位手机号或客户代码"}};var validateboxDefaults={rules:DefaultValidateRules,validateOnCreate:false,missingMessage:"必填项，不可为空"}
...
$.extend($.fn.linkbutton,$.fn.linkbutton0);self.m_enhanceFn[".my-combobox"]=function(jo){jo.mycombobox();};self.m_enhanceFn[".wui-form-table"]=enhanceTableLayout;function enhanceTableLayout(jo){var tbl=jo[0];if(tbl.rows.length==0)
...
opts.isLoaded_=false;opts.urlParams=param;loadOptions();});jo.change(function(){this.value_="";});jo.on("setOption",function(ev,opt){if(opt==null)
