...
{var self=this;self.ctx=self.ctx||{};var mCommon=jdModule("jdcloud.common");mCommon.assert($.fn.combobox,"require jquery-easyui lib.");function getRow(jtbl)
...
function fnOk()
{if(jdlg.hasClass("wui-readonly")){closeDlg(jdlg);return;}
...
showDlg(jdlg,{url:url,title:opt.title,okLabel:BTN_TEXT[mode],validate:mode!=FormMode.forFind,modal:false,reset:doReset,data:load_data,onSubmit:onSubmit,onOk:onOk,objParam:opt});if(mode==FormMode.forSet)
