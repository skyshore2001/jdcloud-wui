...
return false;if(formMode==FormMode.forSet&&opt.url.action&&/.set$/.test(opt.url.action)){var jtbl=jdlg.jdata().jtbl;var obj=opt.url.action.replace(".set","");var rv=batchOp(obj,"setIf",jtbl,data,function(){closeDlg(jdlg);});if(rv!==false)
return;}
...
var dlgId=jdlg.selector.substr(1);var sel="#tpl_"+dlgId;var html=$(sel).html();if(html){loadDialogTpl(html,dlgId,pageFile);return true;}
...
reload(jtbl,undefined,param);onCrud();return;}
...
{param=$.extend({},{fmt:"excel",pagesz:-1},param);if(ac==null){setTimeout(function(){ac=jtbl.datagrid("options").url;});}
return function(){var debugShow=m_batchMode;var p1=getQueryParamFromTable(jtbl,param);var url=WUI.makeUrl(ac,p1);console.log("export: "+url);console.log("(HINT: debug via Ctrl-Export OR window.open=$.get)");if(debugShow){$.get(url);return;}
window.open(url);}}
...
return;loadOptions();return false;});jo.change(function(){this.value_="";});}
