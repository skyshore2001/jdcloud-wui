...
callType+="-mock";var method=(postParams==null?'GET':'POST');var opt={dataType:'text',url:url,data:postParams,type:method,xhrFields:{withCredentials:true},success:fn,ctx_:ctx};if(window.FormData&&postParams instanceof FormData){opt.processData=false;opt.contentType=false;}
...
var dlgId=jdlg.selector.substr(1);var pageFile=getModulePath(dlgId+".html");$.ajax(pageFile).then(function(html){var jcontainer=$("#my-pages");var jo=$(html).filter("div");if(jo.size()>1||jo.size()==0){console.log("!!! Warning: bad format for dialog '"+selector+"'. Element count = "+jo.size());jo=jo.filter(":first");}
...
if(mode==FormMode.forFind&&jd.mode!=FormMode.forFind){jfrm.find(":input[name]").each(function(i,e){var je=$(e);var bak=je.jdata().bak={bgcolor:je.css("backgroundColor"),disabled:je.prop("disabled"),type:null}
if(je.hasClass("notForFind")||je.attr("notForFind")!=null){je.prop("disabled",true);je.css("backgroundColor","");}
else{je.prop("disabled",false);je.css("backgroundColor","#ffff00");var type=je.attr("type");if(type&&["number","date","time","datetime"].indexOf(type)>=0){bak.type=type;je.attr("type","text");}}});jfrm.find(".easyui-validatebox").validatebox("disableValidation");}
else if(jd.mode==FormMode.forFind&&mode!=FormMode.forFind){jfrm.find(":input[name]").each(function(i,e){var je=$(e);var bak=je.jdata().bak;je.prop("disabled",bak.disabled);je.css("backgroundColor",bak.bgcolor);if(bak.type){je.attr("type",bak.type);}})
...
jfrm.form("validate");function onOk(retData){if(mode==FormMode.forFind){var param=getFindData(jfrm);mCommon.assert(jd.jtbl);var dgOpt=jd.jtbl.datagrid("options");if(param.cond&&dgOpt&&dgOpt.url&&dgOpt.url.params&&dgOpt.url.params.cond){param.cond=dgOpt.url.params.cond+" AND ("+param.cond+")";}
...
if(opts.dirty||m_dataCache[opts.url]===undefined){self.callSvr(opts.url,onLoadOptions);}
