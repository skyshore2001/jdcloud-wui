...
{var self=this;self.ctx=self.ctx||{};var mCommon=jdModule("jdcloud.common");var m_batchMode=false;self.toggleBatchMode=toggleBatchMode;function toggleBatchMode(val){if(val!==undefined)
m_batchMode=val;else
m_batchMode=!m_batchMode;app_alert("批量模式: "+(m_batchMode?"ON":"OFF"));self.tabMain.toggleClass("batchMode",m_batchMode);var opt=WUI.getActivePage().find(".datagrid-f").datagrid("options");opt.ctrlSelect=!m_batchMode;$.fn.datagrid.defaults.singleSelect=false;$.fn.datagrid.defaults.ctrlSelect=!m_batchMode;}
...
var idMap={};$.each(data1.rows,function(i,e){idMap[e.id]=true;});$.each(data1.rows,function(i,e){var fatherId=e[fatherField];if(fatherId&&(idMap[fatherId]||parentId)){e._parentId=fatherId;}
...
{jtbl[datagrid]("loaded");var idx=jtbl[datagrid]("getRowIndex",rowData);var objArr=jdListToArray(data);if(datagrid=="treegrid"){$.extend(rowData,objArr[0]);if(rowData["_parentId"]&&rowData["_parentId"]!=rowData["fatherId"]){rowData["_parentId"]=rowData["fatherId"];jtbl.treegrid("remove",rowData.id);jtbl.treegrid("append",{parent:rowData["_parentId"],data:[rowData]});}
else{jtbl.treegrid("update",{id:rowData.id,row:rowData});}
return;}
...
return;var row=objArr[0];if(datagrid=="treegrid"){jtbl.treegrid('append',{parent:row["fatherId"],data:[row]});return;}
...
if(initfn)
{console.log("### initfn: "+attr);try{initfn.apply(jo,paramArr||[]);}catch(ex){console.error(ex);throw(ex);}}
jo.jdata().init=true;}
...
function initPage()
{var title0=jpage.attr("title")||"无标题";if(title==null)
title=title0;else
title=title.replace('%s',title0);var force=false;if(title.substr(-1,1)=="!"){force=true;title=title.substr(0,title.length-1);}
var tt=self.tabMain;if(tt.tabs('exists',title)){if(!force){tt.tabs('select',title);return;}
tt.tabs('close',title);}
var id=tabid(title);var content="<div id='"+id+"' title='"+title+"' />";var closable=(pageName!=self.options.pageHome);tt.tabs('add',{title:title,closable:closable,fit:true,content:content});var jtab=$("#"+id);var jpageNew=jpage.clone().appendTo(jtab);jpageNew.addClass('wui-page');jpageNew.attr("wui-pageName",pageName);jpageNew.attr("title",title);$.parser.parse(jpageNew);enhanceGrid(jpageNew);self.enhanceWithin(jpageNew);callInitfn(jpageNew,paramArr);jpageNew.data("showPageArgs_",showPageArgs_);jpageNew.trigger('pagecreate');jpageNew.trigger('pageshow');}
...
opt=$.extend({okLabel:"确定",cancelLabel:"取消",noCancel:false,modal:true,reset:true,validate:true},opt);jdlg.addClass('wui-dialog');callInitfn(jdlg,[opt]);var jfrm=jdlg.is("form")?jdlg:jdlg.find("form:first");var formMode=jdlg.jdata().mode;jfrm.trigger("beforeshow",[formMode,opt]);var btns=[{text:opt.okLabel,iconCls:'icon-ok',handler:fnOk}];if(!opt.noCancel)
btns.push({text:opt.cancelLabel,iconCls:'icon-cancel',handler:fnCancel})
if($.isArray(opt.buttons))
btns.push.apply(btns,opt.buttons);var small=self.isSmallScreen();var dlgOpt=$.extend({maximizable:!small,collapsible:!small,resizable:!small,left:null,top:null,closable:!opt.noCancel,modal:opt.modal,buttons:btns,title:opt.title},opt.dialogOpt);if(jdlg.is(":visible")){dlgOpt0=jdlg.dialog("options");$.extend(dlgOpt,{left:dlgOpt0.left,top:dlgOpt0.top});}
jdlg.dialog(dlgOpt);var perm=jdlg.attr("wui-perm")||jdlg.dialog("options").title;jdlg.toggleClass("wui-readonly",(opt.objParam&&opt.objParam.readonly)||!self.canDo(perm,"对话框"));jdlg.okCancel(fnOk,opt.noCancel?undefined:fnCancel);if(opt.reset)
...
setFixedFields(jdlg,opt);focusDlg(jdlg);jfrm.trigger("show",[formMode,opt.data]);function fnCancel(){closeDlg(jdlg)}
function fnOk()
...
return false;var newData={};var dfd=self.triggerAsync(jfrm,"validate",[formMode,opt.data,newData]);if(dfd===false)
...
if(formMode==FormMode.forSet&&opt.url.action&&/.set$/.test(opt.url.action)){var jtbl=jdlg.jdata().jtbl;var obj=opt.url.action.replace(".set","");var rv=batchOp(obj,"setIf",jtbl,data,function(){closeDlg(jdlg);});if(rv!==false)
return;}
self.callSvr(opt.url,success,data);}
else{success(data);}}
function success(data)
{if(data!=null&&opt.onOk){jfrm.trigger('retdata',[data,formMode]);opt.onOk.call(jdlg,data);}}}}
var tmrBatch_;$(document).keydown(function(e){if(e.ctrlKey||e.metaKey){m_batchMode=true;clearTimeout(tmrBatch_);tmrBatch_=setTimeout(function(){m_batchMode=false;tmrBatch_=null;},500);}});$(window).keyup(function(e){if(e.ctrlKey||e.metaKey){m_batchMode=false;clearTimeout(tmrBatch_);}});self.batchOp=batchOp;function batchOp(obj,ac,jtbl,data,onBatchDone,forceFlag)
{if(obj==null||jtbl==null)
return false;var selArr=jtbl.datagrid("getChecked");if(!forceFlag&&!(m_batchMode||selArr.length>1)){return false;}
var acName;if(ac=="setIf"){acName="批量更新";}
else if(ac=="delIf"){acName="批量删除";}
else{return;}
var queryParams;var doBatchOnSel=selArr.length>1&&selArr[0].id!=null;if(!doBatchOnSel&&forceFlag===2&&selArr.length==1&&selArr[0].id!=null)
doBatchOnSel=true;if(doBatchOnSel){var idList=$.map(selArr,function(e){return e.id}).join(',');queryParams={cond:"t0.id IN ("+idList+")"};confirmBatch(selArr.length);}
else{var dgOpt=jtbl.datagrid("options");var p1=dgOpt.url&&dgOpt.url.params;var p2=dgOpt.queryParams;queryParams=$.extend({},p1,p2);if(!queryParams.cond)
queryParams.cond="t0.id>0";var p3=$.extend({},queryParams,{res:"count(*) cnt"});self.callSvr(obj+".query",p3,function(data1){confirmBatch(data1.d[0][0]);});}
return;function confirmBatch(batchCnt){console.log(obj+"."+ac+": "+JSON.stringify(queryParams));if(!$.isFunction(data)){app_confirm(acName+batchCnt+"条记录？",function(b){if(!b)
return;doBatch(data);});}
else{var dataFn=data;data=dataFn(batchCnt);if(data==false)
return;$.when(data).then(doBatch);}}
function doBatch(data){if(ac=="setIf"&&$.isEmptyObject(data)){app_alert("没有需要更新的内容。");return;}
self.callSvr(obj+"."+ac,queryParams,function(cnt){onBatchDone&&onBatchDone();reload(jtbl);app_alert(acName+cnt+"条记录");},data);}}
function setFixedFields(jdlg,beforeShowOpt){jdlg.find(".wui-fixedField[name]").each(function(){var je=$(this);var name=je.attr("name");var fixedVal=beforeShowOpt&&beforeShowOpt.objParam&&beforeShowOpt.objParam[name];if(fixedVal||fixedVal==''){je.attr("readonly",true);var forAdd=beforeShowOpt.objParam.mode==FormMode.forAdd;if(forAdd){je.val(fixedVal);}}
else{je.attr("readonly",null);}});}
...
self.canDo=canDo;function canDo(topic,cmd,defaultVal)
{if(!g_data.permSet)
return true;self.assert(topic);if(defaultVal===undefined)
defaultVal=true;if(cmd==null){var rv=g_data.permSet[topic];if(rv!==undefined)
return rv;return defaultVal;}
var rv=g_data.permSet[topic+"."+cmd];if(rv!==undefined)
return rv;rv=g_data.permSet[cmd];if(rv!==undefined)
return rv;rv=g_data.permSet[topic+".只读"];if(rv===undefined)
rv=g_data.permSet["只读"];if(rv&&(cmd=="新增"||cmd=="修改"||cmd=="删除"||cmd=="导出"||cmd=="对话框")){return false;}
rv=g_data.permSet[topic];if(rv!==undefined)
return rv;return defaultVal;}
var BTN_TEXT=["添加","保存","保存","查找","删除"];function getFindData(jfrm)
{var kvList={};var kvList2={};self.formItems(jfrm,function(ji,name,it){if(ji.hasClass("notForFind"))
return;var v=it.getValue(ji);if(v==null||v==="")
return;if(ji.hasClass("wui-notCond"))
kvList2[name]=v;else
kvList[name]=v;})
...
var dlgId=jdlg.selector.substr(1);var arr=dlgId.split("__");var tplName=arr[0];var sel="#tpl_"+tplName;var html=$(sel).html();if(html){loadDialogTpl(html,dlgId,pageFile);return true;}
...
var param=getFindData(jo);if(!force&&$.isEmptyObject(param)){console.warn("doFind: no param");return;}
var dgOpt=jtbl.datagrid("options");if(!appendFilter||$.isEmptyObject(dgOpt.queryParams)){if(param.cond&&dgOpt&&dgOpt.url&&dgOpt.url.params&&dgOpt.url.params.cond){param.cond=dgOpt.url.params.cond+" AND ("+param.cond+")";}}
else{if(dgOpt.queryParams.cond)
dgOpt.queryParams.cond+=" AND ("+param.cond+")";$.extend(param,dgOpt.queryParams);}
...
jdlg=$(jdlg);else
jdlg.data("objParam",jdlg.objParam);if(loadDialog(jdlg,onLoad))
return;function onLoad(){showObjDlg(jdlg,mode,opt);}
callInitfn(jdlg,[opt]);if(opt.jtbl){jdlg.jdata().jtbl=opt.jtbl;}
...
{mCommon.assert(jd.jtbl);if(mode==FormMode.forDel){var rv=batchOp(obj,"delIf",jd.jtbl,null,onCrud);if(rv!==false)
return;}
...
reload(jd.jtbl);self.app_show('删除成功!');onCrud();});});return;}
var jfrm=jdlg.is("form")?jdlg:jdlg.find("form:first");var doReset=!(jd.mode==FormMode.forFind&&mode==FormMode.forFind)
if(mode==FormMode.forFind&&jd.mode!=FormMode.forFind){jfrm.find(":input[name], .textbox-text").each(function(i,e){var je=$(e);var bak=je.jdata().bak={disabled:je.prop("disabled"),title:je.prop("title"),type:null}
...
else if(jd.mode==FormMode.forFind&&mode!=FormMode.forFind){jfrm.find(":input[name], .textbox-text").each(function(i,e){var je=$(e);var bak=je.jdata().bak;if(bak==null)
return;je.prop("disabled",bak.disabled);je.removeClass("wui-find-field");je.prop("title",bak.title);if(bak.type){je.attr("type",bak.type);}})
jfrm.find(".easyui-validatebox").validatebox("enableValidation");}
jd.mode=mode;var load_data;if(mode==FormMode.forAdd){var init_data=jd.init_data||(jd2&&jd2.init_data);load_data=$.extend({},init_data);if(jd.jtbl&&isTreegrid(jd.jtbl)&&(rowData=getRow(jd.jtbl,true))){if(rowData.state=="open"){load_data["fatherId"]=rowData.id;}
...
function onOk(retData){var jtbl=jd.jtbl;if(mode==FormMode.forFind){mCommon.assert(jtbl);doFind(jfrm,jtbl);onCrud();return;}
...
{closeDlg(jdlg);}
if(!opt.offline)
self.app_show('操作成功!');onCrud();}
function onCrud(){if(obj&&!opt.offline){console.log("refresh: "+obj);$(".my-combobox").trigger("markRefresh",obj);}
opt.onCrud&&opt.onCrud();}}
self.dg_toolbar=dg_toolbar;function dg_toolbar(jtbl,jdlg)
{var toolbar=jtbl.jdata().toolbar||"rfasd";var btns=[];var btnSpecArr=toolbar.split("");for(var i=2;i<arguments.length;++i){btnSpecArr.push(arguments[i]);}
var jpage=jtbl.closest(".wui-page");var perm=jpage.attr("wui-perm")||jpage.attr("title");var ctx={jpage:jpage,jtbl:jtbl,jdlg:jdlg};for(var i=0;i<btnSpecArr.length;++i){var btn=btnSpecArr[i];if(!btn)
...
if(btn.text!="-"&&perm&&!self.canDo(perm,btn["wui-perm"]||btn.text)){continue;}
btns.push(btn);}
return btns;}
$.extend(dg_toolbar,{r:function(ctx){return{text:'刷新',iconCls:'icon-reload',handler:function(){reload(ctx.jtbl,null,m_batchMode?{}:null);}}},f:function(ctx){return{text:'查询',iconCls:'icon-search',handler:function(){showObjDlg(ctx.jdlg,FormMode.forFind,{jtbl:ctx.jtbl});}}},a:function(ctx){return{text:'新增',iconCls:'icon-add',handler:function(){showObjDlg(ctx.jdlg,FormMode.forAdd,{jtbl:ctx.jtbl});}}},s:function(ctx){return{text:'修改',iconCls:'icon-edit',handler:function(){showObjDlg(ctx.jdlg,FormMode.forSet,{jtbl:ctx.jtbl});}}},d:function(ctx){return{text:'删除',iconCls:'icon-remove',handler:function(){showObjDlg(ctx.jdlg,FormMode.forDel,{jtbl:ctx.jtbl});}}},'export':function(ctx){return{text:'导出',iconCls:'icon-save',handler:getExportHandler(ctx.jtbl)}}});self.dg_dblclick=function(jtbl,jdlg)
...
throw"error: bad datagrid: \""+jtbl.selector+"\"";ac=jtbl.datagrid("options").url;}
var p1=getQueryParamFromTable(jtbl,param);var debugShow=false;if(m_batchMode){var fmt=prompt("输入导出格式: excel csv txt excelcsv html (以!结尾为调试输出)",p1.fmt);if(!fmt)
...
{var opt=jtbl.datagrid("options");param=$.extend({},opt.queryParams,param);var selArr=jtbl.datagrid("getChecked");if(selArr.length>1&&selArr[0].id!=null){var idList=$.map(selArr,function(e){return e.id}).join(',');param.cond="t0.id IN ("+idList+")";}
if(param.orderby===undefined&&opt.sortName){param.orderby=opt.sortName;if(opt.sortOrder&&opt.sortOrder.toLowerCase()!="asc")
...
if(param.fname===undefined){param.fname=jtbl.prop("title")||jtbl.closest(".wui-page").prop("title");if(opt.queryParams&&opt.queryParams.cond){var keys=[];opt.queryParams.cond.replace(/'([^']+?)'/g,function(ms,ms1){keys.push(ms1);});if(keys.length>0){param.fname+="-"+keys.join("-");}}}
return param;}
window.YesNoMap={0:"否",1:"是"};var Formatter={dt:function(value,row){var dt=WUI.parseDate(value);if(dt==null)
return value;return dt.format("L");},number:function(value,row){return parseFloat(value);},atts:function(value,row){if(value==null)
return"(无)";return value.toString().replace(/(\d+)(?::([^,]+))?,?/g,function(ms,attId,name){var url=WUI.makeUrl("att",{id:attId});if(name==null)
name=attId;return"<a target='_black' href='"+url+"'>"+name+"</a>&nbsp;";});},pics1:function(value,row){if(value==null)
return"(无图)";return'<a target="_black" href="'+WUI.makeUrl("pic",{id:value})+'">'+value+'</a>';},pics:function(value,row){if(value==null)
return"(无图)";var maxN=Formatter.pics.maxCnt||3;value1=value.toString().replace(/(\d+)(?::([^,]+))?,?/g,function(ms,picId,name){if(name==null)
name="图"+picId;if(maxN<=0)
return name+" ";--maxN;var url=WUI.makeUrl("att",{id:picId});return'<img alt="'+name+'" src="'+url+'">';});var linkUrl=WUI.makeUrl("pic",{id:value});return'<a target="_black" href="'+linkUrl+'">'+value1+'</a>';},flag:function(yes,no){if(yes==null)
yes="是";if(no==null)
...
return v;}},enumStyler:function(colorMap){return function(value,row){var color=colorMap[value];if(Color[color])
...
return;var val=typeof(showField)=="string"?row[showField]:showField?(row[field]+"-"+value):value;return self.makeLinkTo(dlgRef,row[field],val);}}};self.formatter=Formatter;$.extend($.fn.combobox.defaults,{valueField:'val',textField:'text'});function dgLoader(param,success,error)
...
self.doSpecial(jo,'td',function(ev){var jo=$(this).next();if(jo.find("[name]").size()>0){var appendFilter=(ev.ctrlKey||ev.metaKey);doFind(jo,null,appendFilter);}},3,2);function dup(s,n){var ret='';for(var i=0;i<n;++i){ret+=s;}
...
var icon={i:"info",w:"warning",e:"error"}[type];var s={i:"提示",w:"警告",e:"出错"}[type]||"";var s1="<b>["+s+"]</b>";jmsg=$.messager.alert(self.options.title+" - "+s,s1+" "+msg,icon,fn);setTimeout(function(){var jbtn=jmsg.find(".l-btn");jbtn.focus();if(alertOpt.timeoutInterval){setTimeout(function(){try{jbtn.click();}catch(ex){console.error(ex);}},alertOpt.timeoutInterval);}});}
...
return;jpage.trigger('pagedestroy');}});var jmenu=$('<div><div id="mnuReload">刷新页面</div><div id="mnuBatch">批量模式</div></div>');jmenu.menu({onClick:function(mnuItem){var mnuId=mnuItem.id;switch(mnuItem.id){case"mnuReload":self.reloadPage();self.reloadDialog(true);break;case"mnuBatch":self.toggleBatchMode();break;}}});function onSpecial(ev){jmenu.menu('show',{left:ev.pageX,top:ev.pageY});return false;}
self.doSpecial(self.tabMain.find(".tabs-header"),".tabs-selected",onSpecial,3);self.tabMain.find(".tabs-header").on("contextmenu",".tabs-selected",onSpecial);var tmr;$(window).on("resize",function(){if(tmr)
...
return;if(opts.jdEnumMap||opts.jdEnumList){loadOptions();opts.isLoaded_=true;}
else if(opts.url){o.enableAsyncFix=true;loadOptions();if(!jo.attr("ondblclick"))
{jo.off("dblclick").dblclick(function(){if(!confirm("刷新数据?"))
return false;refresh();});}
jo.on("refresh",refresh);jo.on("markRefresh",markRefresh);jo.on("loadOptions",function(ev,param){opts.urlParams=param;loadOptions();});jo.click(function(){if(opts.isLoaded_)
return;loadOptions();return false;});jo.change(function(){this.value_="";});jo.keydown(function(){if($(this).attr("readonly"))
return false;});}
function loadOptions()
{jo.prop("value_",jo.val());jo.empty();var j1=$("<option value=''></option>").appendTo(jo);if(opts.emptyText)
...
if(opts.url==null)
return;var url=opts.url;if($.isFunction(url)){if(url.length==0){url=url();}
...
{var hook=$.valHooks["select"];$.valHooks["select"]={set:function(elem,value){elem.value_=value;return hook.set.apply(this,arguments);},get:function(elem){if(elem.enableAsyncFix)
