...
kvList[e.name]=v;})
var cond=self.getQueryParam(kvList);if(kvList2)
...
param.pagesz=9999;return function(){var url=WUI.makeUrl(ac,getParamFromTable(jtbl,param));window.open(url);}}
self.getParamFromTable=getParamFromTable;function getParamFromTable(jtbl,param)
{if(param==null)
param={};var opt=jtbl.datagrid("options");$.extend(param,opt.queryParams);if(param.orderby===undefined&&opt.sortName){param.orderby=opt.sortName;if(opt.sortOrder&&opt.sortOrder.toLowerCase()!="asc")
...
res+=',';res+=e.field+" "+e.title;});param.res=res;}
return param;}
...
if(opts.url){loadOptions();if(!jo.attr("ondblclick"))
...
$("<option value=''></option>").appendTo(jo);if(opts.dirty||m_dataCache[opts.url]===undefined){self.callSvrSync(opts.url,applyData);}
...
var arr=$.isArray(data.d)?mCommon.rs2Array(data):$.isArray(data)?data:data.list;mCommon.assert($.isArray(arr),"bad data format for combobox");$.each(arr,function(i,row){var jopt=$("<option></option>").attr("value",row[opts.valueField]).text(getText(row)).appendTo(jo);});}
function refresh()
{var val=jo.val();markRefresh();loadOptions();jo.val(val);}
