...
{var self=this;self.ctx=self.ctx||{};var mCommon=jdModule("jdcloud.common");var m_batchMode=false;self.isBatchMode=function(){return m_batchMode||(window.event?(window.event.ctrlKey||window.event.metaKey):false);}
self.toggleBatchMode=toggleBatchMode;function toggleBatchMode(val){if(val!==undefined)
m_batchMode=val;else
m_batchMode=!m_batchMode;app_alert(T("批量模式")+": "+(m_batchMode?"ON":"OFF"));self.tabMain.toggleClass("batchMode",m_batchMode);var jtbl=WUI.getActivePage().find(".datagrid-f");if(jtbl.size()==0)
return;var opt=jtbl.datagrid("options");opt.ctrlSelect=!m_batchMode;$.fn.datagrid.defaults.singleSelect=false;$.fn.datagrid.defaults.ctrlSelect=!m_batchMode;}
...
var PAGING_NO_TOTAL=99999999;function jdListToDgList(data)
{if(!data)
data=[];var ret=data;if($.isArray(data)){ret={total:data.length,rows:data};}
else if($.isArray(data.list)){ret={total:data.total,rows:data.list};}
else if(data.h!==undefined)
{var arr=mCommon.rs2Array(data);ret={total:data.total,rows:arr};}
if(ret.total===undefined){if(data.nextkey){ret.total=PAGING_NO_TOTAL;}
else{ret.total=ret.rows.length;}}
return ret;}
...
return;var initfn;try{initfn=eval(attr);}
...
if(self.isBatchMode()){showPageOpt.force=true;}
if(window.event&&window.event.shiftKey){showPageOpt.forceNew=true;}
if(paramArr==null){paramArr=[showPageOpt];}
else if($.isPlainObject(paramArr)){$.extend(true,showPageOpt,paramArr);paramArr=[showPageOpt];}
...
showPageOpt.title=title0;var title=showPageOpt.title;title=T(title).replace('%s',title0);if(showPageOpt.forceNew){if(window.showPageForceNewCnt==null)
window.showPageForceNewCnt=1;else
++window.showPageForceNewCnt;title+="_"+(window.showPageForceNewCnt);}
var tt=showPageOpt.target?$("#"+showPageOpt.target):self.tabMain;if(tt.tabs('exists',title)){if(!showPageOpt.force){tt.tabs('select',title);dfdShowPage.resolve();return;}
tt.tabs('close',title);}
var id=tabid(title);var content="<div id='"+id+"' title='"+title+"' style='height:100%'/>";var jtab=$(content);var closable=(pageName!=self.options.pageHome);tt.tabs('add',{title:title,closable:closable,fit:true,content:jtab,selected:showPageOpt.selected});var jpageNew=jpage.clone().appendTo(jtab);jpageNew.addClass('wui-page');jpageNew.attr("wui-pageName",pageName);jpageNew.attr("title",title);var dep=self.evalAttr(jpageNew,"wui-deferred");if(dep){self.assert(dep.then,"*** wui-deferred attribute DOES NOT return a deferred object");dep.then(initPage1);return;}
...
else{var datagrid=isTreegrid(jtbl)?"treegrid":"datagrid";var dgOpt=jtbl[datagrid]("options");var p1=dgOpt.url&&dgOpt.url.params;if(p1){p1=$.extend(true,{},p1);var arr=["_app","res","orderby","fname","page","pagesz","fmt"];arr.forEach(function(e){delete p1[e];});}
var p2=!ignoreQueryParam&&dgOpt.queryParams;}
return self.extendQueryParam({},p1,p2,p3,param);}
self.getPageOpt=getPageOpt;function getPageOpt(jpage)
{var showPageArgs=jpage.data("showPageArgs_");return showPageArgs&&showPageArgs[1];}
self.getPageFilter=getPageFilter;function getPageFilter(jpage,name)
{var showPageOpt=getPageOpt(jpage);if(showPageOpt&&$.isPlainObject(showPageOpt.pageFilter)){var ret=showPageOpt.pageFilter;if(name){ret=$.isPlainObject(ret.cond)?ret.cond[name]:null;if(!isFixedField(ret))
ret=null;}
return ret;}}
...
return false;var data=$.extend(true,{},jdlg.data("origin_"));jdlg.find(".wui-subobj").each(function(){var jsub=$(this);var subOpt=WUI.getOptions(jsub);if(!(subOpt.valueField&&subOpt.dlg&&subOpt.dgCall&&subOpt.relatedKey))
return;subOpt.forceLoadData();var ms=subOpt.relatedKey.match(/^([^ =]+)/);if(!ms)
...
return;rows1.forEach(function(e){delete e.id;delete e[relatedKey];});subOpt.forceLoadData();subOpt.dgCall("loadData",rows);},50);});jdlg.trigger("duplicate",[data]);delete data.id;WUI.showObjDlg(jdlg,FormMode.forAdd,{data:data});}
...
return;var btns=[{id:"btnOk",text:opt.okLabel,iconCls:'',handler:fnOk}];if(!opt.noCancel)
btns.push({id:"btnCancel",text:opt.cancelLabel,iconCls:'',handler:fnCancel})
if($.isArray(opt.buttons))
btns.unshift.apply(btns,opt.buttons);var small=self.isSmallScreen();var dlgOpt=$.extend({maximizable:!small,collapsible:!small,resizable:!small,left:null,top:null,closable:!opt.noCancel,modal:opt.modal,buttons:btns,title:opt.title},opt.dialogOpt,WUI.getOptions(jdlg));if(jdlg.is(":visible")){var dlgOpt0=jdlg.dialog("options");$.extend(dlgOpt,{left:dlgOpt0.left,top:dlgOpt0.top});}
var h=jdlg[0].style.height;if(h==""||h=="auto"){dlgOpt.shadow=false;}
jdlg.dialog(dlgOpt);var perm=jdlg.attr("wui-perm")||jdlg.dialog("options").title;jdlg.toggleClass("wui-readonly",(opt.objParam&&opt.objParam.readonly)||!self.canDo(perm,"对话框")||(formMode==FormMode.forSet&&!self.canDo(perm,"修改")));jdlg.parent().addClass("wui-fullscreen");jdlg.okCancel(fnOk,opt.noCancel?undefined:fnCancel);if(opt.reset)
...
self.batchOp=batchOp;function batchOp(obj,ac,jtbl,opt)
{if(obj==null)
...
return false;}
var selArr=jtbl.datagrid("getChecked");var batchOpMode=opt.batchOpMode;if(!batchOpMode&&!(self.isBatchMode()||selArr.length>1)){return false;}
if(batchOpMode===2&&!self.isBatchMode()&&selArr.length==0){self.app_alert(T("请先选择一行。"),"w");return false;}
var doBatchOnSel=selArr.length>1&&(selArr[0].id!=null||opt.offline);if(!doBatchOnSel&&batchOpMode===2&&!self.isBatchMode()&&selArr.length==1&&selArr[0].id!=null)
...
self.setDlgReadonly=setDlgReadonly;function setDlgReadonly(jdlg,fn)
{jdlg.on("beforeshow",function(ev,formMode,dlgOpt){if(formMode!=FormMode.forSet)
return;var val=fn(dlgOpt.data);if(val!==undefined){var objParam=jdlg.prop("objParam");objParam.readonly=val;}});}
self.showObjDlg=showObjDlg;function showObjDlg(jdlg,mode,opt)
{if(jdlg.constructor!=jQuery)
jdlg=$(jdlg);if(window.event&&window.event.shiftKey&&(mode==FormMode.forAdd||mode==FormMode.forSet||mode==FormMode.forFind)&&jdlg.selector[0]=='#'){if(window.showDlgForceNewCnt==null)
window.showDlgForceNewCnt=1;else
++window.showDlgForceNewCnt;var sel=jdlg.selector+"_inst_"+window.showDlgForceNewCnt;jdlg=$(sel);}
if(loadDialog(jdlg,onLoad,opt))
...
$.extend(dg_toolbar,{r:function(ctx){return{text:'刷新',iconCls:'icon-reload',handler:function(){reload(ctx.jtbl,null,self.isBatchMode()?{}:null);return false;}}},f:function(ctx){return{text:'查询',class:'splitbutton',iconCls:'icon-search',handler:function(){showObjDlg(ctx.jdlg,FormMode.forFind,{jtbl:ctx.jtbl});return false;},menu:self.createFindMenu(ctx.jtbl)}},a:function(ctx){return{text:'新增',iconCls:'icon-add',handler:function(){showObjDlg(ctx.jdlg,FormMode.forAdd,{jtbl:ctx.jtbl});return false;}}},s:function(ctx){return{text:'修改',iconCls:'icon-edit',handler:function(){showObjDlg(ctx.jdlg,FormMode.forSet,{jtbl:ctx.jtbl});return false;}}},d:function(ctx){return{text:'删除',iconCls:'icon-remove',handler:function(){showObjDlg(ctx.jdlg,FormMode.forDel,{jtbl:ctx.jtbl});return false;}}},'export':function(ctx){var handler=getExportHandler(ctx.jtbl);return{text:'导出',class:'splitbutton',iconCls:'icon-save',handler:handler,menu:createExportMenu(handler)}},dup:function(ctx){return{text:'复制',iconCls:'icon-add',handler:function(){self.GridHeaderMenu.dup(ctx.jtbl);return false;}}}});self.createExportMenu=createExportMenu;function createExportMenu(handler)
{var jmenu=$('<div>'+'<div data-options="id:\'csv\'">逗号分隔(csv)</div>'+'<div data-options="id:\'txt\'">TAB分隔(txt)</div>'+'<div data-options="id:\'html\'">网页(html)</div>'+'</div>');jmenu.menu({onClick:function(item){console.log(item);handler({exportParam:{fmt:item.id}});}})
return jmenu;}
self.dg_dblclick=function(jtbl,jdlg)
{var t0;return function(idx,data){var t1=new Date();if(t0&&t1-t0<500){console.warn('ignore duplicate dblclick');return;}
t0=t1;showObjDlg(jdlg,FormMode.forSet,{jtbl:jtbl});}}
self.m_enhanceFn["a[href]"]=enhanceAnchor;function enhanceAnchor(jo)
{if(jo.attr("onclick"))
return;if(jo.attr("target"))
return;var title=jo.text().trim();jo.click(function(ev){var showPageOpt={title:title};var href=$(this).attr("href");if(href[0]=='!'){href=href.substr(1);showPageOpt.force=1;}
if(href.search(/^#(page\w+)$/)>=0){var pageName=RegExp.$1;WUI.showPage.call(this,pageName,showPageOpt);return false;}
if(href.match(/^(http|[\.\/])/)){if(ev.shiftKey){setTimeout(function(){window.open(href);});}
else{WUI.showPage("pageIframe",showPageOpt,[href]);}
return false;}});}
self.getExportHandler=getExportHandler;function getExportHandler(jtbl,ac,param)
{return function(ev){var p0=$.extend({},{fmt:"excel",pagesz:-1},param,ev&&ev.exportParam);if(ac==null){if(jtbl.size()==0||!jtbl.hasClass("datagrid-f"))
throw"error: bad datagrid: \""+jtbl.selector+"\"";var datagrid=isTreegrid(jtbl)?"treegrid":"datagrid";ac=jtbl[datagrid]("options").url;if(ac==null){app_alert("该数据表不支持导出","w");return false;}}
var p1=getQueryParamFromTable(jtbl,p0);if(p1.gres&&!(p0&&p0.res))
p1.gresHidden=1;var debugShow=false;if(self.isBatchMode()){var fmt=prompt("输入导出格式: excel csv txt excelcsv html outfile(无导出条数限制), 以!结尾为调试输出",p1.fmt);if(!fmt)
return false;if(fmt.substr(-1)=="!"){fmt=fmt.substr(0,fmt.length-1);debugShow=true;}
p1.fmt=fmt;}
var url=WUI.makeUrl(ac,p1);console.log("export: "+url);console.log("(HINT: debug via Ctrl-Export OR window.open=$.get)");if(debugShow){$.get(url);return false;}
window.open(url);}}
...
if(param.res===undefined){var resArr=self.getDgCols(opt,function(e){var one=e.field;if(one!=e.title||e.jdEnumMap){if(/[\s()\[\]\{\}\/\\,<>.!@#$%^&*-+]|^\d/.test(e.title))
one+=" "+Q(e.title,'"');else
one+=" "+e.title;}
if(e.jdEnumMap){one+='='+mCommon.kvList2Str(e.jdEnumMap,';',':');}
return one;});param1.res=resArr.join(',');}
if(param.fname===undefined){param1.fname=jtbl.prop("title")||jtbl.closest(".wui-page").prop("title");}
return param1;}
self.getDgColMap=getDgColMap;function getDgColMap(jtbl_or_opt)
{var map={};getDgCols(jtbl_or_opt,function(e){map[e.field]=e;});return map;}
self.getDgCols=getDgCols;function getDgCols(jtbl_or_dgOpt,fn)
{var dgOpt=jtbl_or_dgOpt;if(jtbl_or_dgOpt instanceof jQuery){var jtbl=jtbl_or_dgOpt;var datagrid=WUI.isTreegrid(jtbl)?"treegrid":"datagrid";dgOpt=jtbl[datagrid]("options");}
var ret=[];$.each([dgOpt.frozenColumns[0],dgOpt.columns[0]],function(idx0,cols){if(cols==null)
return;$.each(cols,function(i,e){if(!e.field||e.field.substr(-1)=="_")
return;if(fn){ret.push(fn(e));}
else{ret.push(e);}});});return ret;}
...
if(res==null)
res={};res.isTreegrid=self.isTreegrid(jtbl);var datagrid=res.datagrid=(res.isTreegrid?"treegrid":"datagrid");var opt=res.opt=jtbl[datagrid]("options");res.url=opt.url;res.param=opt.queryParams;res.ac=opt.url&&opt.url.action;if(res.ac){var m=res.ac.match(/^([A-Z]\w*)(\.query)?$/);res.obj=m&&m[1];}
if(res.sel!==undefined){res.sel=jtbl[datagrid]('getSelected');}
if(res.selArr!==undefined){res.selArr=jtbl[datagrid]("getChecked");}
if(res.res!==undefined){res.res=getDgColMap(opt);}
...
return"(无图)";var value1;if(!/^\d+([,:]|$)/.test(value)){var url="../"+value;value1='<img alt="'+value+'" src="'+url+'">';return'<a target="_black" href="'+url+'">'+value1+'</a>';}
if(!opt.preview)
...
return;value=Math.ceil(value*100);var w=(value>100?100:value);var htmlstr='<div class="easyui-progressbar progressbar" style="min-width: 100px;width: 100%; height: 20px;">'
...
{var jo=$(this);var datagrid=self.isTreegrid(jo)?"treegrid":"datagrid";var opts=jo[datagrid]("options");if(opts.url==null)
...
{var ret=jdListToDgList(data);var isOnePage=(ret.total==ret.rows.length);var doHidePager=(isOnePage&&ret.total<=5);var jtbl=$(this);var dgOpt=jtbl.datagrid("options");if(dgOpt.pagination){var jpager=jtbl.datagrid("getPager");if(ret.total==PAGING_NO_TOTAL&&!dgOpt.pagingNoTotal){jpager.pagination({layout:['list','sep','first','prev','manual','next','sep','refresh'],afterPageText:'页'});dgOpt.pagingNoTotal=true;}
if(dgOpt.pagingNoTotal&&isOnePage){var pagerOpt=jpager.pagination('options');if(pagerOpt.pageNumber!=1){ret.total=pagerOpt.pageNumber*pagerOpt.pageSize;}}
...
return s;return s+parseFloat(v);},0);stat[field]=self.myround(stat[field],6);});ret.footer=[stat];}
return ret;function hidePagerAuto(jtbl,jpager,doHidePager){var tmpName="_restorefn_pager";var isHidden=jpager.css("display")=="none";if(doHidePager&&!isHidden){var h=jpager.height();jpager.hide().height(0);jtbl.data(tmpName,function(){jpager.height(h).show();})}
...
$.fn.datagrid.defaults_bak=$.extend({},$.fn.datagrid.defaults);$.extend($.fn.datagrid.defaults,{rownumbers:true,ctrlSelect:true,pagination:true,pageSize:20,pageList:[20,50,100],loadFilter:dgLoadFilter,loader:dgLoader,onLoadError:self.ctx.defAjaxErrProc,onBeforeSortColumn:function(sort,order){var jtbl=$(this);resetPageNumber(jtbl);},onLoadSuccess:function(data){if(data.total||($.isArray(data)&&data.length>0)){$(this).datagrid("fitColumns");}
else{var body=$(this).data().datagrid.dc.body2;var view=$(this).data().datagrid.dc.view;var h=50;view.height(view.height()-body.height()+h);body.height(h);body.find('table tbody').empty().append('<tr><td width="'+body.width()+'px" height="50px" align="center" class="noData" style="border:none; color:#ccc; font-size:14px">没有数据</td></tr>');}},onHeaderContextMenu:function(ev,field){var jtbl=$(this);var jmenu=GridHeaderMenu.showMenu({left:ev.pageX,top:ev.pageY},jtbl,field);ev.preventDefault();},onClickCell:function(row,field,val){var this_=this;var jtbl=$(this);if(this_.infoTmr)
clearTimeout(this_.infoTmr);this_.infoTmr=setTimeout(function(){GridHeaderMenu.statSelection(jtbl,row,field);delete this_.infoTmr;},1000);},onInitOptions:function(opt){var ac=opt.url&&opt.url.action||(opt.url1&&opt.url1.action);if(ac){var m=ac.match(/\w+(?=\.query\b)/);var obj=m&&m[0];if(obj){opt.obj=obj;var jtbl=$(this);var baseObj=jtbl.attr("my-obj")||obj;var meta=self.UDF.onGetMeta(baseObj);if(meta&&!jtbl.data("udfLoaded_")){self.UDF.addColByMeta(opt.columns[0],meta);jtbl.data("udfLoaded_",true);}}}
if(this.datagridOpt){$.extend(opt,this.datagridOpt);}
opt.columns[0].forEach(function(e){e.title=T(e.title);});},});var GridHeaderMenu={showMenu:function(pos,jtbl,field){var jmenu=$('<div class="mnuGridHeader"></div>');var items=field?GridHeaderMenu.itemsForField:GridHeaderMenu.items;$.each(items,function(i,e){var je=$(e);var perm=je.attr("wui-perm")||je.text().trim();if(canDo("通用",perm))
je.appendTo(jmenu);});jmenu.menu({onClick:function(mnuItem){GridHeaderMenu[mnuItem.id].call(mnuItem,jtbl,field);}});jmenu.menu('show',pos);},items:['<div id="showDlgFieldInfo">字段信息</div>','<div id="filterGrid" data-options="iconCls:\'icon-filter\'">数据筛选</div>','<div id="showDlgDataReport" data-options="iconCls:\'icon-sum\'">自定义报表</div>','<div id="showDlgQuery" data-options="iconCls:\'icon-search\'">高级查询</div>','<div id="dup" wui-perm="新增">再次新增</div>','<div id="import" wui-perm="新增" data-options="iconCls:\'icon-add\'">导入</div>','<div id="export" data-options="iconCls:\'icon-save\'">导出</div>','<div id="copyTable">复制表</div>',],itemsForField:['<div id="copyCol">复制本列</div>','<div id="statCol" data-options="iconCls:\'icon-sum\'">统计本列</div>','<div id="doFindCell" data-options="iconCls:\'icon-filter\'">筛选本列</div>','<div id="freezeCol" data-options="iconCls:\'icon-lock\'">冻结列</div>',],showDlgFieldInfo:function(jtbl){var param=WUI.getQueryParamFromTable(jtbl);console.log(param);var title="字段信息";var title1=jtbl.prop("title")||jtbl.closest(".wui-page").prop("title");if(title1)
...
if(param.orderby)
strArr.push("<b>[排序]</b>\n"+param.orderby);var resArr=$.map(param.res.split(','),function(e){var a=e.split2(' ');return{name:a[0],title:a[1]};});var rows,rowStartIdx;if(datagrid=="datagrid"){rows=jtbl.datagrid("getData").rows;var sel=jtbl.datagrid("getSelected");rowStartIdx=sel?jtbl.datagrid("getRowIndex",sel):0;}
else{rows=jtbl.treegrid("getData");var sel=jtbl.treegrid("getSelected")||rows[0];if(sel){var row=$.extend({},sel);delete row.children;delete row.state;delete row.checkState;delete row.checked;delete row._parentId;rows=[row];}
rowStartIdx=0;}
if(rows.length>0){var colMap={};$.each(resArr,function(i,res){colMap[res.name]=true;addExample(rows,res,rowStartIdx);});$.each(rows[rowStartIdx],function(field,val){if(colMap[field])
return;var res={name:field,title:"(未使用)"};addExample(rows,res,rowStartIdx);resArr.push(res);});}
strArr.push("<b>[字段列表]</b>\n");var jdlg=$("<div class='dlgFieldInfo' style='width:500px;height:500px' title='"+title+"'><pre>"+strArr.join("\n\n")+"</pre>"+"<table id='res' style='width:100%'></table>"+"</div>");if($("#styleFieldInfo").size()==0){var style="<style id='styleFieldInfo'>"+".dlgFieldInfo .datagrid-cell { white-space: normal }"+".dlgFieldInfo .example { color: blue; font-size: 0.8em }"+"</style>";$(style).appendTo(document.head);}
...
function addExample(rows,res,rowStartIdx){var field=res.name;if(rows[rowStartIdx][field]===undefined){res.name+="<span style='color:red'>(缺失)</span>";}
if(res.title){res.title="<span>"+res.title+"</span>";}
var arr=[];var MAX_EXAMPLE=2;for(var i=rowStartIdx;i<rows.length;++i){var row=rows[i];var s=row[field];if($.isArray(s)||$.isPlainObject(s))
s=JSON.stringify(s);if(s===null)
s="(null)";else if(s===undefined)
s="(undefined)";arr.push(s);if(arr.length==MAX_EXAMPLE)
break;}
if(arr.length>0)
res.example="<span class='example'>"+arr.join('<br>')+"</span>";}},filterGrid:function(jtbl){var param=self.getDgInfo(jtbl).param;app_alert(T("筛选条件(cond)? "),"p",function(cond){WUI.reload(jtbl,null,{cond:cond});},{defValue:param&&param.cond});},showDlgDataReport:function(jtbl){self.showDlg("#dlgDataReport");},showDlgQuery:function(jtbl){var data=null;var datagrid=WUI.isTreegrid(jtbl)?"treegrid":"datagrid";var url=jtbl[datagrid]("options").url;if(url&&url.action)
...
DlgImport.show({obj:param.obj},function(){WUI.reload(jtbl);});},'export':function(jtbl){var fn=getExportHandler(jtbl);fn();},dup:function(jtbl){var param=self.getDgInfo(jtbl);if(!param.obj){app_alert(T("该数据表不支持再次新增"),"w");return;}
var rows=jtbl.datagrid("getSelections");if(rows.length==0){app_alert("请选择要复制的行","w");return;}
var idArr=$.map(rows,function(e){return e.id}).sort();app_alert("确认要复制"+rows.length+"行数据？将根据所选行复制并生成新行。","q",function(){dupObj();});function dupObj(){callSvr(param.obj+".dup",{id:idArr.join(',')},function(res){WUI.reload(jtbl);});}},copyTable:function(jtbl){var rows=GridHeaderMenu.getRows(jtbl);var colArr=self.getDgCols(jtbl);var arr=rows.map(function(row){var a=colArr.map(function(e){var v=row[e.field];if(e.jdEnumMap&&e.formatter){v=e.formatter(v);}
return v;});return a.join("\t");});var title=colArr.map(function(e){return e.title;}).join("\t");var ret=title+"\r\n"+arr.join("\r\n");self.execCopy(ret);},dgStatCol:function(rows,field){var stat={cnt:0,realCnt:0,numCnt:0,sum:0,max:0,min:0,info:[]};stat.cnt=rows.length;if(stat.cnt<1)
...
stat.min=v;}});stat.info=["COUNT="+stat.cnt,"COUNT(非空项)="+stat.realCnt];if(stat.numCnt>0){stat.info.push("COUNT(数值项)="+stat.numCnt,"SUM="+self.myround(stat.sum),"MAX="+stat.max,"MIN="+stat.min,"AVG="+self.myround(stat.sum/stat.numCnt))}
return stat;},statSelection:function(jtbl,row,field){var rows=jtbl.datagrid("getSelections");if(rows.length<2)
return;var stat=GridHeaderMenu.dgStatCol(rows,field);var colTitle=jtbl.datagrid("getColumnOption",field).title;var title="选中"+rows.length+"行，列=["+colTitle+"]";app_show(stat.info.join("<br>"),title);},getRows:function(jtbl){var datagrid=WUI.isTreegrid(jtbl)?"treegrid":"datagrid";var rows=jtbl[datagrid]("getSelections");var dgOpt=jtbl[datagrid]("options");if(rows.length<2){var data=jtbl[datagrid]("getData");if(datagrid=="treegrid"){rows=[];getTreeData(data,rows);}
else{rows=data.rows;}}
return rows;function getTreeData(treeRows,rows){treeRows.forEach(function(e){rows.push(e);if($.isArray(e.children)){getTreeData(e.children,rows);}});}},copyCol:function(jtbl,field){var rows=GridHeaderMenu.getRows(jtbl);var colOpt=jtbl.datagrid("getColumnOption",field);var arr=rows.map(function(row){var v=row[field];if(colOpt.jdEnumMap&&colOpt.formatter){v=colOpt.formatter(v);}
return v;});arr.unshift(colOpt.title);var ret=arr.join("\r\n");self.execCopy(ret);},statCol:function(jtbl,field){var data=jtbl.datagrid("getData");var colTitle=jtbl.datagrid("getColumnOption",field).title;if(data.rows.length==0){var info="["+colTitle+"]列: 无数据";app_alert(info);return;}
...
var title=T("查询结果");if(param.fname){title+="-"+param.fname;};WUI.showPage("pageSimple",title+"!",[url,null,null,showChartParam]);}});jdlg.find(".example").css({backgroundColor:"#eeeeee",cursor:"pointer"}).click(function(){$(this).closest("td").find("textarea:first").val($(this).text());});}
$.fn.treegrid.defaults_bak=$.extend({},$.fn.treegrid.defaults);$.extend($.fn.treegrid.defaults,{idField:"id",treeField:"id",pagination:false,rownumbers:true,fatherField:"fatherId",singleSelect:false,ctrlSelect:true,loadFilter:function(data,parentId){if($.isArray(data)&&data[0]&&$.isArray(data[0].children))
return data;var opt=$(this).treegrid("options");var isLeaf=opt.isLeaf;var ret=jdListToTree(data,opt.idField,opt.fatherField,parentId,isLeaf);return ret;},loader:dgLoader,onBeforeLoad:function(row,param){if(row){var opt=$(this).treegrid("options");param.cond=opt.fatherField+"="+row.id;delete param["id"];}},onLoadSuccess:function(row,data){$.fn.datagrid.defaults.onLoadSuccess.call(this,data);},onHeaderContextMenu:$.fn.datagrid.defaults.onHeaderContextMenu});$.extend($.fn.combotree.defaults,{idField:"id",textField:"name",fatherField:"fatherId",loadFilter:function(data,parentId){var arr=null;if($.isArray(data)){arr=data;}
...
var DefaultValidateRules={number:{validator:function(v){return/^[0-9.-]+$/.test(v);},message:T("validate.number","必须为数字!")},idcard:{validator:checkIdCard,message:T("validate.idcard","18位身份证号有误!")},uname:{validator:function(v){return v.length>=4&&v.length<=16&&/^[a-z]\w+$/i.test(v);},message:T("validate.uname","4-16位英文字母或数字，以字母开头，不能出现符号.")},pwd:{validator:function(v){return(v.length>=4&&v.length<=16)||v.length==32;},message:T("validate.pwd","4-16位字母、数字或符号.")},equalTo:{validator:function(v,param){return v==$(this).closest("form").find(param[0]).val();},message:T("validate.equalTo","两次输入不一致.")},cellphone:{validator:function(v){return v.length==11&&!/\D/.test(v);},message:T("validate.cellphone","手机号为11位数字")},datetime:{validator:function(v){return/\d{4}-\d{1,2}-\d{1,2}(\d{1,2}:\d{1,2}(:\d{1,2})?)?/.test(v);
 ...
 usercode: {
  validator: function (v) {
   return /^[a-zA-Z]/.test(v)||(v.length==11&&!/\D/.test(v));},message:T("validate.usercode","11位手机号或客户代码")}};var validateboxDefaults={rules:DefaultValidateRules,validateOnCreate:false,missingMessage:T("必填项，不可为空")}
...
{if(location.search){window.g_args=mCommon.parseQuery(location.search.substr(1));}}
function T(s,def){if(s==null||LANG==null)
return def||s;var s1=s;if(s.length>2&&s.substr(-2)=="管理")
s1=s.substr(0,s.length-2);return LANG[s1]||def||s;}
...
self.dfdLogin.resolve();self.showPage(self.options.pageHome,self.options.pageHomeOpt);if(location.hash.startsWith("#page")){WUI.showPage(location.hash.replace('#',''));}}
self.initClient=initClient;function initClient(param)
{self.callSvrSync('initClient',function(data){g_data.initClient=data;Plugins.plugins_=data.plugins||{};$.each(Plugins.plugins_,function(k,e){if(e.js){var js=BASE_URL+'plugin/'+k+'/'+e.js;mCommon.loadScript(js,{async:true});}});},param);if(g_data.initClient&&g_data.initClient.userInfo){WUI.handleLogin(g_data.initClient.userInfo);}}
self.setApp=setApp;function setApp(app)
...
return;jpage.trigger('pagedestroy');}});self.PageHeaderMenu={items:['<div id="mnuReload" data-options="iconCls:\'icon-reload\'">刷新页面</div>','<div id="mnuReloadDlg">刷新对话框</div>','<div id="mnuBatch">批量模式</div>','<div id="mnuCloseTabs" data-options="iconCls:\'icon-clear\'">关闭其它页</div>','<div id="mnuCloseTabs2">关闭右侧页</div>','<div id="mnuFullscreen" title="Ctrl+Alt+双击">全屏</div>'],mnuReload:function(){self.reloadPage();self.reloadDialog(true);},mnuReloadDlg:function(){var jdlg=self.isBatchMode()?true:null;self.reloadDialog(jdlg);},mnuBatch:function(){self.toggleBatchMode();},mnuCloseTabs:function(doCloseRight){var jtabs=self.tabMain;var curIdx=jtabs.tabs("getTabIndex",jtabs.tabs("getSelected"));var arr=jtabs.tabs("tabs");for(var i=arr.length-1;i>curIdx;--i){jtabs.tabs("close",i);}
if(doCloseRight)
return;for(var i=curIdx-1;i>0;--i){jtabs.tabs("close",i);}},mnuCloseTabs2:function(){self.PageHeaderMenu.mnuCloseTabs(true);},mnuFullscreen:function(){var jp=self.getActivePage();if(jp.size()>0)
jp[0].requestFullscreen();}};var jmenu=null;function onSpecial(ev){if(jmenu==null){jmenu=$('<div>'+self.PageHeaderMenu.items.join('')+'</div>');jmenu.menu({onClick:function(mnuItem){self.PageHeaderMenu[mnuItem.id].call(mnuItem);}});}
...
var m_dataCache={};$.fn.mycombobox=mycombobox;function mycombobox(userOpts)
{var mCommon=jdModule("jdcloud.common");this.each(initCombobox);function initCombobox(i,o)
{var jo=$(o);var opts=WUI.getOptions(jo);var force=userOpts&&userOpts.force;if(!force&&opts.isLoaded_)
return;if(jo.attr("required"))
opts.required=true;if(o.tagName!="SELECT"){var jo1=$("<select></select>");$.each(o.attributes,function(i,e){jo1.attr(e.name,e.value);});jo.replaceWith(jo1);jo=jo1;o=jo1[0];opts=WUI.getOptions(jo,opts);}
$.extend(opts,userOpts);jo.removeAttr("data-options");jo.addClass("easyui-validatebox");jo.validatebox(opts);o.enableAsyncFix=true;if(opts.url){if(!jo.attr("ondblclick"))
...
{if(opts.isLoaded_)
return;opts.isLoaded_=true;if(!(opts.jdEnumMap||opts.jdEnumList||opts.url))
return;jo.prop("value_",jo.val());jo.empty();var j1=$("<option value=''></option>").appendTo(jo);if(opts.emptyText)
j1.text(opts.emptyText);if(opts.jdEnumList){opts.jdEnumMap=parseEnumList(opts.jdEnumList);}
...
opts.url_=url;}
if(m_dataCache[url]===undefined){WUI.callSvr(url,onLoadOptions);}
...
WUI.parseEnumList=parseEnumList;function parseEnumList(s)
{return WUI.parseKvList(s,/[; ]/,/[:=]/,false,function(kv){for(var i=0;i<kv.length;++i)
